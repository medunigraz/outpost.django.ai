...
class ChatCompletionsView(CsrfExemptMixin, AuthenticationMixin, View):
            ...
            with session.send(
                ...
                if stream:
                    ...
                    models.Usage.objects.create(
                        model=im.model, token=token, total=usage.get("total_tokens")
                    )
...
class CompletionsView(CsrfExemptMixin, AuthenticationMixin, View):
    def post(self, request, token, *args, **kwargs):
        body = json.loads(request.body)
        name = body.get("model")
        stream = body.get("stream", False)
...
class ModelsView(CsrfExemptMixin, AuthenticationMixin, JSONResponseMixin, View):
...
class EmbeddingsView(CsrfExemptMixin, AuthenticationMixin, View):
    def post(self, request, token, *args, **kwargs):
        body = json.loads(request.body)
        name = body.get("model")
        im = (
